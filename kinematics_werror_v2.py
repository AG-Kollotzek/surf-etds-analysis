...
class SurfKinematicsNominalV2:
    """Uncertainty-free twin of SurfKinematics (v2) - identical equations, plain numpy.

    WHY THIS EXISTS
        DataConverter._refine_time_alignment needs a fast reference curve to fit the ETD time
        axis against. The QA repo imports `kinematics.SurfKinematics` for this - but that file
        is a STALE nominal model that disagrees with the whole `kinematics_werror*` line:

            kinematics.py:      y_local = -(radius + hAxis_vertical)*sin(p) + rollOffset + h*cos(p)
                                z_local =  radius*(1 - cos(p)) + h*sin(p)      [QA copy: -radius*...]
                                z_global = -z_local
            werror / werror_v2: y_local = -(radius*sin(p))      + rollOffset - h*cos(p)
                                z_local = -(radius*(1 - cos(p)))             - h*sin(p)
                                z_global =  z_local

        (v2's `radius` already contains hAxis_vertical, so the sin(p) term itself matches.)

        The h*cos(p) term has the OPPOSITE SIGN and it dominates True_Longitudinal whenever the
        H axis moves - i.e. exactly the horizontal-axis runs. Measured on meas_04 (pure H sweep,
        +/-10 mm): correlation between the ETD longitudinal trace and kinematics.py is -0.9996
        (RMSE 12.0 mm), against kinematics_werror_v2 it is +0.9996 (RMSE 0.18 mm). Fitting the
        ETD time axis against that mirrored reference would drive the optimiser to a
        meaningless shift. In the QA repo this stayed latent because refine defaults to False
        and its deflection runs are V-axis dominated.

        Using this class keeps the alignment reference and the exported kinematics on the SAME
        model. Uncertainties are dropped only for speed; the nominal values are identical to
        `unp.nominal_values(SurfKinematics(...).calculate_task_space(...))`.
    """

    def __init__(self, sd_calib=None):
        src = SurfKinematics(sd_calib=sd_calib)
        self.hAxis_horizontal = float(unp.nominal_values(src.hAxis_horizontal))
        self.hAxis_vertical = float(unp.nominal_values(src.hAxis_vertical))
        self.hAxis_diagonal = float(unp.nominal_values(src.hAxis_diagonal))
        self.radius = float(unp.nominal_values(src.radius))

    def calculate_task_space(self, h_raw, v_raw, r_raw_deg, couch_angle_raw_deg=0.0):
        h_u = np.asarray(h_raw, dtype=float)
        v_u = np.asarray(v_raw, dtype=float)
        r_u = np.deg2rad(np.asarray(r_raw_deg, dtype=float))

        alpha_offset = np.arctan(self.hAxis_vertical / self.hAxis_horizontal)
        # Clip guards against round-off pushing the argument marginally outside [-1, 1].
        sin_arg = np.clip((self.hAxis_vertical + v_u) / self.hAxis_diagonal, -1.0, 1.0)
        pitch_rad_local = np.arcsin(sin_arg) - alpha_offset

        rollOffset = self.hAxis_horizontal - np.sqrt(
            np.maximum(self.hAxis_diagonal ** 2 - (self.hAxis_vertical + v_u) ** 2, 0.0))

        y_local = -(self.radius * np.sin(pitch_rad_local)) + rollOffset - (h_u * np.cos(pitch_rad_local))
        x_local = h_u * 0.0
        z_local = -(self.radius * (1 - np.cos(pitch_rad_local))) - (h_u * np.sin(pitch_rad_local))

        sin_p, cos_p = np.sin(pitch_rad_local), np.cos(pitch_rad_local)
        sin_r, cos_r = np.sin(r_u), np.cos(r_u)
        pitch_local_out = np.degrees(np.arctan2(sin_p * cos_r, cos_p))
        roll_local_out = np.degrees(np.arcsin(np.clip(sin_p * sin_r, -1.0, 1.0)))
        yaw_local_out = np.degrees(np.arctan2(cos_p * sin_r, cos_r))

        gamma = np.deg2rad(couch_angle_raw_deg)
        cos_g, sin_g = np.cos(gamma), np.sin(gamma)

        return {
            'True_Lateral': x_local * cos_g - y_local * sin_g,
            'True_Longitudinal': x_local * sin_g + y_local * cos_g,
            'True_Vertical': z_local,
            'True_Pitch': pitch_local_out,
            'True_Roll': roll_local_out,
            'True_Yaw': yaw_local_out - float(couch_angle_raw_deg),
        }
